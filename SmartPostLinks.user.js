...
async function processLinksOnPage() {
  ...
  const postLinksToProcess = $('a[href!="#"]', '#mainbar, #chat, #transcript, #content, #sidebar').filter(function () {
    return /\/(questions|q|a|posts|staging-ground)\/\d+/i.test(this.href) && // only post links
      ...
      !$(this).closest('.s-post-summary--content-title, .votecell, .post-menu, .post-signature, .user-info, .comment-date, .post-stickyheader, .onebox, #hot-network-questions, #qtoc').length; // not a child element of these containers
