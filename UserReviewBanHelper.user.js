...
<td class="pl4">${tally.count366}</td>
  ...
  // Mod user history - review bans filter
    ...
    $.get(`${location.origin}/users/history/${uid2}?type=Review+suspension+notice+viewed`, function (data) {
      ...
      $('#user-history .js-review-susp-viewed').each(function (i, el) {
        const aDate = new Date(el.children[0].children[0].title).getTime();
        const bDate = new Date(el.nextSibling.children[0].children[0].title).getTime();
      ...
      if (isSuperuser) {
        ...
        if (location.hash.includes('|/review/triage/')) {
          ...
          if (reviewAction == 'needs-community-edit') {
            $('#canned-messages a[data-message*="Needs community edit"]').trigger('click');
          }
        }
        ...
        // If suggested-edits reviews
          ...
          if (reviewAction == 'approve') {
            $('#canned-messages a[data-message^="You approved poor edits"]').trigger('click');
          }
        }
      }
