...
function declinePostFlags(pid) {
  return new Promise(function (resolve, reject) {
    ...
    $.post({
      url: `${location.origin}/messages/delete-moderator-messages/${pid}/${StackExchange.moderator.renderTimeTicks}?valid=false`,
      data: {
        fkey: fkey,
        comment: 2
      }
...
function sortPosts(filter) {
  ...
  if (filter === 'flagger-rank' && $('#load-flagger-stats').length > 0) {
    $('#load-flagger-stats').trigger('click');
...
function doPageLoad() {
  ...
  $('.js-resolve-action[data-type="decline"]').on('click', function (evt) {
    ...
    setTimeout(() => {
      const opt = flagOpts.find('input[name="dismiss-reason"][value="2"]:visible').get(0)?.click();
