        ...
        getPostComments(pids).then(function(comments) {
            questions.each(function() {
                ...
                $(this).find('.statscontainer').append(modonly ?
                     `<div class="views" title="${cmmts.length} recent comments"><a href="https://${location.hostname}/a/${pid}">${cmmts.length} comments<a/></div>` :
        ...
        .finally(function() {
            ...
            getPostRevisions(pids).then(function(revisions) {
                questions.each(function() {
                    ...
                    const flagCount = $(this).find('.bounty-indicator-tab').hide().text();
