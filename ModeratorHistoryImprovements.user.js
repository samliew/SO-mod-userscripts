...
// @match        https://*.mathoverflow.net/admin/history/*
// @match        https://*.stackapps.com/admin/history/*
// @match        https://*.stackexchange.com/admin/history/*
// @match        https://stackoverflowteams.com/c/*/admin/history/*
//
...
/* globals StackExchange, MS, monthsOfYear */
...
function processNewItems($items) {
  ...
  $('ul li', $items).each(function () {
    ...
    if (/Comment deleted:/.test(t)) {
      ...
      t = t.replace(/(\d+)/, `<a href="${location.origin}/users/$1" target="_blank" title="view user">$1</a>`);
      $(this).addClass('mod-destroys');
    }
    else if (/^Moderator merges users/.test(t)) {
      t = t.replace(/(\d+)/g, `<a href="${location.origin}/users/$1" target="_blank" title="view user">$1</a>`);
      $(this).addClass('mod-merges-users');
    }
    else if (/^(Moderator contacts user|See user-message)/.test(t)) {
      t = t.replace(/(\d+)/, `<a href="${location.origin}/users/message/$1" target="_blank" title="view message">$1</a>`);
      $(this).addClass('mod-contacts-user');
    }
    else if (/^(Moderator removes bounty)/.test(t)) {
      t = t.replace(/(\d+)/, `<a href="${location.origin}/questions/$1" target="_blank" title="view question">$1</a>`);
      $(this).addClass('mod-removes-bounty');
    }
    else if (/^(Moderator edits comment)/.test(t)) {
      t = t.replace(/ - on post id = (\d+), comment id = (\d+)/, ` <a href="${location.origin}/posts/comments/$2" target="_blank" title="view comment">$2</a>`);
      $(this).addClass('mod-edits-comment');
    }
    else if (/^Moderator merges tags/.test(t)) {
      t = t.replace(/\[([^\]]+)\]/g, `<a href="${location.origin}/tags/$1/info" class="post-tag" target="_blank" title="view tag">$1</a>`);
      $(this).addClass('mod-merges-tags');
    }
    else if (/^Moderator merges questions/.test(t)) {
      t = t.replace(/(\d+)/g, `<a href="${location.origin}/questions/$1" target="_blank" title="view question">$1</a>`);
...
(function init() {
  ...
  $items.find('.relativetime').each(function () {
    ...
    if (Date.now() - MS.oneDay > d) {
      this.innerText = `${monthsOfYear[d.getMonth()]} ${d.getDate()} at ${('0' + d.getHours()).substr(-2)}:${('0' + d.getMinutes()).substr(-2)}`;
    }
  });
  ...
  setInterval(() => StackExchange.realtime.updateRelativeDates(), 10000);
})();