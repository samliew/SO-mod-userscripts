...
function getUserPii(uid) {
  return new Promise(function (resolve, reject) {
     ...
     */
    if (!confirm(`This action requires loading of PII for user ${uid}. Do you wish to proceed?`)) {
...
// Retrieve PII (real name, email address, created date, and IP address) for two users
function getComparePii(susUid, targetUid) {
  return new Promise(function (resolve, reject) {
    if (typeof susUid === 'undefined' || susUid === null) { reject(); return; }
    if (typeof targetUid === 'undefined' || targetUid === null) { reject(); return; }
     ...
     * - (none yet)
     */
    if (!confirm(`This action requires loading of PII for users ${susUid} & ${targetUid}. Do you wish to proceed?`)) {
      console.error(`getComparePii(${uid}) cancelled.`);
      reject(`getComparePii(${uid}) cancelled.`);
      return;
    }

    $.post(`${location.origin}/admin/data/get-details-for-sus-users?susUserId=${susUid}&targetUserId=${targetUid}`)
      .done(function (response) {
        resolve(response);
      })
      .fail(function (response) {
        console.error(`getComparePii(${uid}) failed: `, response);
        reject(response);
      });
  });
}
...
function getPostsFromApi(postIds, filter = 'default', sort = 'creation', order = 'desc', site = siteApiSlug) {
  return new Promise(function (resolve, reject) {
    ...
    const url = `${_seApiUrl}/posts/${postIds.join(';')}?sort=${sort}&order=${order}&site=${site}&filter=${filter}&key=${_apikey}`;
