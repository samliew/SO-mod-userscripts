...
function sendCmMessage(userId, templateName, postText) {
  return new Promise(function (resolve, reject) {
    if (typeof pid === 'undefined' || pid === null) { reject(); return; }
    if (typeof postText !== 'string' || postText.length < 10) { reject(); return; }
    ...
    $.post({
      ...
      .done(function (response) {
        console.log(`sendCmMessage(${pid}) success: `, response);
        resolve(response);
      })
      .fail(function (response) {
        console.error(`sendCmMessage(${pid}) failed: `, response);
        reject(response);
      });
  });
}
