...
function getUserInfoFromApi(uid, filter = 'default', sort = 'creation', order = 'desc', site = siteApiSlug) {
  return new Promise(function (resolve, reject) {
    ...
    const url = `https://api.stackexchange.com/2.3/users/${uid}?sort=${sort}&order=${order}&site=${site}&filter=${filter}&key=${seApiKey ?? ''}`;
...
function getPostsFromApi(postIds, filter = 'default', sort = 'creation', order = 'desc', site = siteApiSlug) {
  return new Promise(function (resolve, reject) {
    ...
    const url = `https://api.stackexchange.com/2.3/posts/${postIds}?sort=${sort}&order=${order}&site=${site}&filter=${filter}&key=${seApiKey ?? ''}`;
