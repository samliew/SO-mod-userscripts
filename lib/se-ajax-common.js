...
function getUserInfoFromApi(uid, filter = 'default', sort = 'creation', order = 'desc', site = siteApiSlug) {
  return new Promise(function (resolve, reject) {
    ...
    // Use this default filter (excludes collectives info)
    // https://api.stackexchange.com/docs/users-by-ids#order=desc&sort=activity&filter=!2.M(VZShqC0fJZpZgpaoz&site=stackoverflow
    filter = filter === 'default' ? '!2.M(VZShqC0fJZpZgpaoz' : filter;

    // Use a default apikey if none provided
    _apikey = typeof apikey === 'string' ? apikey : '';

    const url = `https://api.stackexchange.com/2.3/users/${uid}?sort=${sort}&order=${order}&site=${site}&filter=${filter}&key=${_apikey}`;
...
function getPostsFromApi(postIds, filter = 'default', sort = 'creation', order = 'desc', site = siteApiSlug) {
  return new Promise(function (resolve, reject) {
    ...
    // Use this default filter (excludes collectives info), adds post title (even on answers)
    // https://api.stackexchange.com/docs/posts-by-ids#order=desc&sort=activity&filter=!Sgvr(CKKANacp9uvCA&site=stackoverflow
    filter = filter === 'default' ? '!Sgvr(CKKANacpAgSgj' : filter;

    // Use a default apikey if none provided
    _apikey = typeof apikey === 'string' ? apikey : '';

    const url = `https://api.stackexchange.com/2.3/posts/${postIds}?sort=${sort}&order=${order}&site=${site}&filter=${filter}&key=${_apikey}`;
