...
function getUserInfoFromApi(uid, filter = 'default', sort = 'creation', order = 'desc', site = siteApiSlug) {
  return new Promise(function (resolve, reject) {
    ...
    const _seApiUrl = typeof seApiUrl === 'string' ? seApiUrl : 'https://api.stackexchange.com/2.3';
...
async function getPostTimeline(postId) {
  ...
  const dom = makeElemFromHtml(html);
...
async function getLatestPostRevisionGuid(postId) {
  ...
  const html = await resp.text();
  const dom = makeElemFromHtml(html);
  return dom.querySelector('.js-revision a[href$="/view-source"]')?.getAttribute('href')?.match(/\/revisions\/([a-z0-9-]+)\/view-source/)?.pop();
}
...
async function getRevisionSource(guidOrUrl) {
  ...
  // Get revision body
  const revisionBody = dom.querySelector('pre');
  if (!revisionBody)
    return console.error(`No revision body found for ${postId}!`);

  // Simple strip markdown formatting
  return revisionBody.textContent
    .replace(/^\s*#{1,3}\s*/gm, '\n\n') // remove headings
    .replace(/!?\[(.*?)\]\(.*?\)/g, '$1') // remove links and images
    .replace(/`{1,3}(.*?)`{1,3}/gs, '$1') // remove code
    .replace(/^(?:\s*(-|\d+)\s*)+/gm, '') // remove lists
    .replace(/^(?:\s*>\s*)+/gm, '') // remove blockquotes
    .replace(/[*_]{1,2}(.*?)[*_]{1,2}/gs, '$1') // Remove bold and italics
    .trim();
}
