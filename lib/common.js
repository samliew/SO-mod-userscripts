...
(() => {
  ...
  const bannerClass = '.js-announcement-banner';
  const bannerOpts = {
  ...
  const makeAnnouncementBanner = html => {
    // Hide other banners
    document.querySelectorAll(bannerClass).forEach(el => el.remove());

    // Create and insert after header nav
    const bannerEl = document.createElement('div');
    bannerEl.id = "announcement-banner";
    bannerEl.classList.add(...'js-announcement-banner somu-announcement-banner bg-black-700 fc-white ff-sans fs-body2 py2'.split(/\s+/));
    bannerEl.innerHTML = `
      <div class="d-flex jc-space-between wmx12 mx-auto px16 py8">
        <div class="flex--item mr12">${html}</div>
        <span class="d-none flex--item fc-white js-dismiss" title="dismiss"><svg aria-hidden="true" class="m0 svg-icon iconClear" width="18" height="18" viewBox="0 0 18 18"><path d="M15 4.41 13.59 3 9 7.59 4.41 3 3 4.41 7.59 9 3 13.59 4.41 15 9 10.41 13.59 15 15 13.59 10.41 9 15 4.41Z"></path></svg></span>
      </div>`;
    const header = document.querySelector('header.js-top-bar');
    header?.insertAdjacentElement('afterEnd', bannerEl); // header may not exist
  };
  ...
  setTimeout(() => {
    const bannerEl = document.querySelector(bannerClass);
    const annVisible = !!(bannerEl.offsetWidth || bannerEl.offsetHeight || bannerEl.getBoundingClientRect().width);
