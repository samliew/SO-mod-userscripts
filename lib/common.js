...
const sessionStore = _window.sessionStorage;
...
const seApiUrl = 'https://api.stackexchange.com/2.2'; // excluding trailing slash
...
const ajaxPromise = (options, type = 'text') => {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (typeof GM_xmlhttpRequest === 'function') {
      try { GM_xmlhttpRequest(options); }
      catch (e) { reject(e); }
    }
    else if (typeof GM !== 'undefined' && typeof GM?.xmlhttpRequest === 'function') {
      try { GM.xmlhttpRequest(options); }
      catch (e) { reject(e); }
    }
    else {
      reject("GM_xmlhttpRequest is not defined.");
    }
  });
};
...
const toShortLink = (url, baseDomain = null) => {
  ...
  // Detect if already short link
  if (/\/[qa]\/\d+/.test(url)) return url;
...
/**
 * @summary Parse HTML document string and return body element
 * @param {string} html HTML string
 * @returns {HTMLElement} body element
 * @example
 *   getDomFromHtml('<div id="foo">Hello World</div>')
 */
const getDomFromHtml = (html) => {
  const el = document.implementation?.createHTMLDocument();
  el.documentElement.innerHTML = html;
  return el.body || el;
};
...
const getCookie = key => {
  ...
  const val = document.cookie?.split("; ").find(kvp => kvp.startsWith(`${key}=`))?.split("=").pop();
 ...
 * @param {string} key cookie name
 * @returns {boolean} whether cookie was deleted
 */
const deleteCookie = key => navigator.cookieEnabled && getCookie(key) && (document.cookie = `${key}=; max-age=-1`) && true;

/**
 * @summary Set a cookie value
 * @param {string} key cookie name
 ...
 * @param {Date} [options.expires] cookie expiry date
 * @param {boolean} [options.secure] whether cookie is secure
 * @param {boolean} [options.httpOnly] whether cookie is httpOnly
 * @param {boolean} [options.sameSite] cookie sameSite policy
 * @returns {boolean} whether cookie was set
 * @example
 *   setCookie('foo', 'bar', { maxAge: 60 * 60 * 24 * 365, secure: true });
 *   setCookie('foo', 'bar', { maxAge: 60 * 60 * 24 * 365, sameSite: 'strict' });
 *   setCookie('foo', 'bar', { maxAge: -1 }); // delete cookie
 *
 * @example
 *   console.log(
 *     setCookie('test', 'abc', {
...
const setCookie = (key, value, options = {}) => {
  const cookie = [
    ...
    options.maxAge ? `max-age=${options.maxAge}` : '', // maxAge takes precedence over expires
    !options.maxAge && options.expires && options.expires instanceof Date ? `expires=${options.expires.toUTCString()}` : '',
    options.secure ? 'secure' : '',
    options.httpOnly ? 'httponly' : '',
    options.sameSite ? `samesite=${options.sameSite}` : ''
  ].filter(Boolean).join('; ');

  return navigator.cookieEnabled && (document.cookie = cookie) && true;
};
...
// SOMU-wide Announcements
(() => {
  // return; // Comment out to enable global announcement

  const isDismissed = getCookie('somu-announcement-dismissed') === 'true';

  // Announcement banner variables
  const bannerOpts = {
    // Announcement banner (HTML)
    html: `A message from SOMU userscripts: We, Stack Exchange moderators, are <a target="_blank" class="s-link js-link fc-orange-400" style="color: var(--orange-400) !important;" href="https://openletter.mousetail.nl">calling for solidarity</a> against actions taken by Stack Overflow, Inc. which is posing a major threat to the integrity and trustworthiness of the platform and its content.`,
    // Chat announcement banner, sidebar pin, and system message (HTML)
    chat: `We, Stack Exchange moderators, are <a target="_blank" class="s-link js-link fc-orange-400" style="color: var(--orange-400) !important;" href="https://openletter.mousetail.nl">calling for solidarity</a> against actions taken by Stack Overflow, Inc. which is posing a major threat to the integrity and trustworthiness of the platform and its content - by SOMU userscripts`,
    // Fallback text
    text: `We, Stack Exchange moderators, are calling for solidarity against actions taken by Stack Overflow, Inc. which is posing a major threat to the integrity and trustworthiness of the platform and its content.`,
    link: 'https://openletter.mousetail.nl',
  };

  const removeAll = selector => document.querySelectorAll(selector).forEach(el => el.remove());

  // Check if our announcement banner does not exist
  // Fallback to an unblockable alert so it can't be missed
  const checkAnnouncementExist = (selector = '.js-announcement-banner', secs = 60) => {
    if (_window.somuAnnounceTimeout) clearTimeout(_window.somuAnnounceTimeout); // Clear timeout if it exists so it only runs once

    _window.somuAnnounceTimeout = setTimeout(() => {
      // Find banner
      const el = document.querySelector(selector);
      const elVisible = !!(!el || el.offsetWidth || el.offsetHeight || el.getBoundingClientRect().width);
      if (!el || !elVisible) {
        alert(bannerOpts.text);
        const a = makeElemFromHtml(`<a href="${bannerOpts.link}" target="_blank" style="display: none;"></a>`);
  ...
  const makeAnnouncementBanner = () => {

    // Banner is temporarily dismissed
    if (isDismissed) return;

    // If document not ready, ignore
    if (!document.readyState || document.readyState !== 'complete') return;

    // Chat
    if (isChat) {

      // Keep starred message pinned in sidebar
      const pinnedEl = makeElemFromHtml(`
        <li class="js-starred-message" style="display: block;">
          <span class="vote-count-container stars">
            <span class="img vote"></span><span class="times"></span>
          </span>
          <span>${bannerOpts.chat}</span>
        </li>`);

      const starredPosts = document.querySelector('#starred-posts ul');
      if (starredPosts) {
        setInterval(() => {
          removeAll('.js-starred-message');
          starredPosts.insertAdjacentElement('afterbegin', pinnedEl);
        }, 10000);
      }

      // Header, on other chat pages
      const bannerEl = makeElemFromHtml(`
        <div class="js-announcement-banner bg-black-700 fc-white ff-sans fs-body2 py2 mt12">
          <div class="d-flex jc-space-between wmx12 mx-auto px16 py8">
            <div class="flex--item w100 lh-unset fs-body1">${bannerOpts.html}</div>
            <a class="flex--item ml12 fc-white js-dismiss" title="dismiss"><svg aria-hidden="true" class="m0 svg-icon iconClear pe-none" width="18" height="18" viewBox="0 0 18 18"><path d="M15 4.41 13.59 3 9 7.59 4.41 3 3 4.41 7.59 9 3 13.59 4.41 15 9 10.41 13.59 15 15 13.59 10.41 9 15 4.41Z"></path></svg></a>
          </div>
        </div>`);

      const chatHeader = document.querySelector('#header');
      if (chatHeader) {
        removeAll('.js-announcement-banner');
        chatHeader.insertAdjacentElement('afterEnd', bannerEl);
        checkAnnouncementExist('.js-announcement-banner');
      }
    }
    // Main, excluding Teams
    else if (!isSOTeams) {
      const bannerEl = makeElemFromHtml(`
        <div class="js-announcement-banner bg-black-700 fc-white ff-sans fs-body2 py2">
          <div class="d-flex jc-space-between wmx12 mx-auto px16 py8">
            <div class="flex--item w100">${bannerOpts.html}</div>
            <a class="flex--item ml12 fc-white js-dismiss" title="dismiss"><svg aria-hidden="true" class="m0 svg-icon iconClear pe-none" width="18" height="18" viewBox="0 0 18 18"><path d="M15 4.41 13.59 3 9 7.59 4.41 3 3 4.41 7.59 9 3 13.59 4.41 15 9 10.41 13.59 15 15 13.59 10.41 9 15 4.41Z"></path></svg></a>
          </div>
        </div>`);

      const header = document.querySelector('header.js-top-bar');
      if (header) {
        removeAll('.js-announcement-banner');
    ...
    // Dismiss announcement banner link click event
    document.addEventListener('click', evt => {
      // Only for links with class js-dismiss
      const el = evt.target;
      if (!el.classList.contains('js-dismiss')) return;

      // Remove check timeout if exists
      if (_window.somuAnnounceTimeout) clearTimeout(_window.somuAnnounceTimeout);

      // Save dismissed state to session
      setCookie('somu-announcement-dismissed', 'true', {
        maxAge: 21600, // 6 hours in seconds
      });

      // Dismiss announcement banners
      removeAll('.js-announcement-banner');
  ...
  // Make announcement banner after delay, for userscripts that run on document-start
  setTimeout(makeAnnouncementBanner, 1000);
})();
