...
(() => {
  ...
  const bannerOpts = {
    ...
    html: `A message from SOMU userscripts: We, Stack Exchange moderators, are <a target="_blank" class="s-link js-link fc-orange-400" href="https://openletter.mousetail.nl">calling for solidarity</a> against actions taken by Stack Overflow, Inc. which is posing a major threat to the integrity and trustworthiness of the platform and its content.`,
    // Chat announcement banner, sidebar pin, and system message (HTML)
    chat: `We, Stack Exchange moderators, are <a target="_blank" class="s-link js-link fc-orange-400" href="https://openletter.mousetail.nl">calling for solidarity</a> against actions taken by Stack Overflow, Inc. which is posing a major threat to the integrity and trustworthiness of the platform and its content - by SOMU userscripts`,
    // Fallback text
  ...
  const makeAnnouncementBanner = () => {
    ...
    if (isChat) {
      ...
      // System message in content
      const chatMsgEl = makeElemFromHtml(`
        <div class="js-system-message my16">
          <div class="ml12 mr4 p12 px24 bg-black-050 fs-body1">${bannerOpts.html}</div>
        </div>`)

      const chatContainer = document.querySelector('#chat, #transcript');
      if (chatContainer) {
        removeAll('.js-system-message');
        chatContainer.insertAdjacentElement("beforeend", chatMsgEl);
        checkAnnouncementExist('.js-system-message');
      }
