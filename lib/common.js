...
const isChat = ['chat.stackexchange.com', 'chat.meta.stackexchange.com', 'chat.stackoverflow.com'].some(v => v === _host);
const isMSE = _host === 'meta.stackexchange.com';
const isSO = _host === 'stackoverflow.com';
const isSOMeta = _host === 'meta.stackoverflow.com';
...
const siteApiSlug = parentUrl.replace(/^https?:\/\//, '').replace(/(\.stackexchange)?\.(com|net|org)$/, '').replace(/\s/g, '');

const userId = (typeof StackExchange !== 'undefined' ? StackExchange.options?.user?.userId : null) || _chatUserId;
const fkey =
