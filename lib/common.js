...
const parentUrl = (typeof StackExchange !== 'undefined' && StackExchange.options?.site?.parentUrl) || location.origin.replace('meta.', '');
 ...
 * @example
 *   injectStyle('body { background-color: red; }');
 */
const injectStyle = (content, atDocumentEnd = true) => {
  const styleElem = makeElem('style', { type: 'text/css' }, content);
