...
(() => {
  ...
  const bannerOpts = {
    ...
    text: `We are calling for solidarity against actions taken by Stack Overflow, Inc. which is posing a major threat to the integrity and trustworthiness of the platform and its content.`,
    link: 'https://openletter.mousetail.nl',
  };

  const removeAllBanners = () => {
    document.querySelectorAll('.js-announcement-banner').forEach(el => el.remove());
  };

  // Check if our announcement banner does not exist
  const checkAnnouncementBanner = (secs = 60) => {
    if (_window.somuAnnounceTimeout) clearTimeout(_window.somuAnnounceTimeout);

    _window.somuAnnounceTimeout = setTimeout(() => {
      // Find banner
      const bannerEl = document.querySelector('.js-announcement-banner');
      const annVisible = !!(!bannerEl || bannerEl.offsetWidth || bannerEl.offsetHeight || bannerEl.getBoundingClientRect().width);
      if (!bannerEl || !annVisible) {
        alert(bannerOpts.text);
        const a = document.createElement('a');
        a.href = bannerOpts.link;
        a.target = "_blank";
        a.click();
      }
    }, secs * 1000);
  ...
  const makeAnnouncementBanner = html => {
    if(isDismissed) return;

    // Chat
    if (isChat) {
      const pinnedEl = makeElemFromHtml(`
        <li class="js-announcement-banner" style="display: block;">
          <span title="pinned announcement" class="vote-count-container stars owner-star user-star">
            <span class="img vote"></span>
            <span class="times"></span>
          </span>${html}
        </li>`);

      const stars = document.querySelector('#starred-posts ul');
      if (stars) {
        removeAllBanners();
        stars.insertAdjacentElement('afterbegin', pinnedEl);
      }

      const bannerEl = makeElemFromHtml(`
        <div id="announcement-banner" class="js-announcement-banner bg-black-700 fc-white ff-sans fs-body2 py2 mt12">
          <div class="d-flex jc-space-between wmx12 mx-auto px16 py8">
            <div class="flex--item w100 lh-unset fs-body1">${html}</div>
    ...
    // Main, excluding Teams
      ...
      const bannerEl = makeElemFromHtml(`
        <div id="announcement-banner" class="js-announcement-banner bg-black-700 fc-white ff-sans fs-body2 py2">
          <div class="d-flex jc-space-between wmx12 mx-auto px16 py8">
            <div class="flex--item w100">${html}</div>
            <a class="flex--item ml12 fc-white js-dismiss" title="dismiss"><svg aria-hidden="true" class="m0 svg-icon iconClear pe-none" width="18" height="18" viewBox="0 0 18 18"><path d="M15 4.41 13.59 3 9 7.59 4.41 3 3 4.41 7.59 9 3 13.59 4.41 15 9 10.41 13.59 15 15 13.59 10.41 9 15 4.41Z"></path></svg></a>
          </div>
        </div>`);
    ...
    // Dismiss announcement banner link event
  ...
  setTimeout(() => makeAnnouncementBanner(bannerOpts.html), 1000);
})();
