...
(() => {
  ...
  const bannerOpts = {
    html: `We are <a target="_blank" class="s-link js-link fc-orange-400" href="https://openletter.mousetail.nl">calling for solidarity</a> against actions taken by Stack Overflow, Inc. which is posing a major threat to the integrity and trustworthiness of the platform and its content.`,
  ...
  const makeAnnouncementBanner = html => {
      ...
      const bannerEl = makeElemFromHtml(`
        <div id="announcement-banner" class="js-announcement-banner bg-black-700 fc-white ff-sans fs-body2 py2">
          <div class="d-flex jc-space-between wmx12 mx-auto px16 py8">
            <div class="flex--item w100 ta-center">${html}</div>
            <span class="flex--item ml12 fc-white js-dismiss" title="dismiss" style="display:none;"><svg aria-hidden="true" class="m0 svg-icon iconClear" width="18" height="18" viewBox="0 0 18 18"><path d="M15 4.41 13.59 3 9 7.59 4.41 3 3 4.41 7.59 9 3 13.59 4.41 15 9 10.41 13.59 15 15 13.59 10.41 9 15 4.41Z"></path></svg></span>
          </div>
        </div>`);
    ...
    // Chat
      ...
      const bannerEl2 = makeElemFromHtml(`
        <div class="user-container monologue js-announcement-banner">
          <div class="signature">
            <div class="username" style="display: block;">Announcement</div>
          </div>
          <div class="messages" style="background-color: lightgoldenrodyellow;">
            <div class="message"><div class="content">${html}</div>
            <span class="flash"><span class="always owner-star stars vote-count-container"><span class="img vote" title="star this message as useful / interesting for the transcript"></span><span class="times"></span></span></span></div>
          </div>
          <div class="clear-both" style="height: 0px;">&nbsp;</div>
        </div>`);

      const stars = document.querySelector('#starred-posts ul');
      const transcript = document.querySelector('#chat, #transcript, #chat-transcript');

      if (stars || transcript) removeOtherBanners();
      if (stars) stars.insertAdjacentElement('afterbegin', bannerEl);
      if (transcript) transcript.insertAdjacentElement('beforeend', bannerEl2);
    }
  };

  // Make announcement banner, after 1s for userscripts that run on document-start
