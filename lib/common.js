...
// Platform / Site (variables to work in chat too)
...
const parentUrl = (
  (typeof StackExchange !== 'undefined' && StackExchange.options?.site?.parentUrl) ||
  _chatSiteLink ||
  (isMSE ? location.origin : location.origin.replace('meta.', ''))
...
// get self user ID from StackExchange object, if available
const selfId = (typeof StackExchange !== 'undefined' ? StackExchange.options?.user?.userId : null) || _chatUserId;
const userId = selfId; // temp backwards compatibility

// get current user ID from url, if on a user profile page
// see https://regex101.com/r/96DIYA
const currentUserId = Number(
  location.pathname.match(
    /^\/(?:users(?:\/)?(?:preferences|account-info|history|flagged-posts|flag-summary|message\/create)?|admin(?:\/)?(?:cm-message\/create|(?:show|xref)-user-(?:votes|ips)))\/(\d+)/i
  )?.[1]
) || null;
 ...
 * @example
 *   getUid('/users/12345/username'); // 12345
 *   getUid('/users/account-info/12345'); // 12345
 */
const getUid = function (url) {
  if (typeof url !== 'string') return null;
  const match = url.match(/\/users\/(?:(?:account-info)\/)?(\d+)/);
