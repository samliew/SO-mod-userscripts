...
.tag-popup .-container,
.tag-popup .-container .-arrow:before,
.tag-popup .-container *:not(button):not(svg) {
...
#chat-body .message .content a[href*="/review/"]:visited:hover,
...
#transcript div.message .content {
...
#chat-body .mine .messages div.message.reply-parent .content {
  background-color: #444;
  border-right: 2px solid yellowgreen;
}
...
addStylesheet(darkModeCss, false); // false = append to head


document.addEventListener('DOMContentLoaded', function () {
  let $ = _window.jQuery || window.jQuery;

  // Add class to body so we can override a few stubborn styles
  ...
  if (location.hostname === "stackoverflow.com") {
    $('.s-topbar .s-topbar--logo .-img').replaceWith(soLogo);
  }
  ...
  // Highlight comment on page load if in URL
  ...
  // Need to be additionally processed when loaded
